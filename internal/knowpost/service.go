...
type KnowPostService struct {
	...
	// bloom 与空值缓存叠加：Cuckoo 前置拦截一定不存在的 ID，软删时可 Delete；nil 表示关闭。
...
// newDetailBloom 按配置装配详情存在性 Cuckoo 过滤器；关闭或依赖缺失时返回 nil。
...
// WarmDetailBloom 从数据库游标扫描未删除知文 ID，批量写入 Cuckoo 过滤器。
//
// 启动时异步调用一次即可；写路径 CreateDraft 与读路径回源成功也会增量 Add；
// 软删路径 Delete 会从过滤器移除。空过滤器时 MightContain fail-open。
