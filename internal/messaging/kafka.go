...
const (
	counterWriterMaxAttempts = 5
	counterBackoffMin        = 200 * time.Millisecond
	counterBackoffMax        = 2 * time.Second
...
//
// 函数调用说明：
//   - NewTopicWriter(cfg, cfg.Topics.CounterEvents, false):
//     委托 NewTopicWriter 创建 writer。async=false 表示等待 broker 确认。
//
// 设计决策：
//
...
//	counter 主题（中高保证）：
//	  - RequiredAcks = RequireAll: 等待所有 ISR 副本确认
//	  - MaxAttempts = 5: 保留有限重试，避免长期阻塞
//	  - WriteBackoff = 200ms~2s: 中等重试间隔
//	  - Async = false: 等待 broker 返回结果
//
// 参数：
//   - cfg:   Kafka 全局配置
//   - topic: 目标 topic 名称
