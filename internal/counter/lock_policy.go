...
func rebuildLockOptions(cfg *config.CounterConfig, logger *zap.Logger) redislock.Options {
	...
	return redislock.Options{
		...
		MaxRenewErrors:         defaultMaxRenewErrors,
		ActiveReleaseOnFailure: true,
		OnRenewFailed: func(err error) {
			logger.Warn("rebuild lock renew failed",
				zap.Error(err),
				zap.Duration("ttl", leaseTTL),
			)
		},
	}
}
