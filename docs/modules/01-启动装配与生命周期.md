...
> 2. 掌握 `InfraDeps` 工厂模式集中管理基础设施（MySQL、Redis、Kafka、FreeCache）连接句柄。
> 3. 掌握后台长跑协程 (Runners) 的集中式注册与托管，杜绝野协程 (Orphan Goroutines) 泄漏。
> 4. 掌握基于系统信号 (SIGINT/SIGTERM) 的 5 阶段优雅停机 (Graceful Shutdown) 与资源回收路径。
> 
> **📚 前置概念预习**
...
6. 组装后台 runner（以 `internal/bootstrap/app.go` 为准）：
   ...
   - hotKeyRunner（热点探测 flush）
   - fanout consumer（Kafka brokers 非空时；订阅 `canal-outbox` 并过滤 `KnowPostPublished`）
...
```mermaid
flowchart TD
    A[加载配置并校验] --> B[创建 InfraDeps<br/>MySQL / Redis / Kafka / 缓存 / 雪花]
...
sequenceDiagram
    ...
    App->>H: 阶段 2: HTTP Server.Shutdown(10s 超时)
...
2. **阶段 2：拒绝新 HTTP 请求并关闭端口**
   - 立即调用 Gin HTTP Server 的 `Shutdown(timeoutCtx)`，指定 10 秒超时窗口。此时服务器停止接收新连接，但会给已有活跃请求 10 秒时间完成处理与写响应。
3. **阶段 3：等待后台 Runner（Consumer/Loop）安全退出**
   - `server.App` 通过 `sync.WaitGroup` 阻塞等待所有注册在案的 `BackgroundRunner`（包含 `AggregationConsumer`、`HotKeyRunner`、`Canal Bridge` 等）。每个 Runner 在收到 `ctx.Done()` 后完成当前批次提交（Commit Offset / Flush 缓存），随后安全退出协程。
...
1. **切忌先关闭 DB/Redis 连接池，再去调用 HTTP `Shutdown`**：
   ...
   - 后果：在 HTTP 服务器 Shutdown 的 10 秒超时窗口内，正在执行的最后一个用户请求访问数据库时会直接触发 `sql: database is closed` 报错异常崩溃！
...
- **思考题 2**：如果一个后台 Consumer 在收到 `ctx.Done()` 信号后，由于某些卡顿逻辑迟迟不退出，系统会永远死锁等待吗？
  - *提示*：优雅停机设置了兜底超时上下文（如 10s 或 15s），超时发生后主进程会强行汇总 `Timeout Error` 并打印日志强行结束，兼顾了优雅性与挂死保护。
