...
| 04 | [知文内容缓存与Feed模块](04-知文内容缓存与Feed模块.md) | `knowpost` + Cuckoo | ★★★★★ |
...
| 09 | [热点缓存与多级缓存策略](09-热点缓存与多级缓存策略.md) | `cache` / HotKey / Cuckoo | ★★★★ |
...
改了主路径逻辑（缓存键、Cuckoo/Bloom、水位、双表、outbox 主题）必须同步对应编号文档。  
...
1. 04 知文：Cuckoo / NULL / 版本失效 / Feed 碎片是否同步  
...
| 内容是否存在 | MySQL `know_posts` | Cuckoo + Redis NULL | DB 回源确认后补 Cuckoo 或写 NULL；软删 Cuckoo Delete |
