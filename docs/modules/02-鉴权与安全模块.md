...
| 会话刷新锁 | Redis `lock:auth:refresh:user:{userID}` | 串行化 Refresh 和 ResetPassword |
...
| `acquireRefreshSessionLock` | redislock.AcquireWithRetry(`lock:auth:refresh:user:{userID}`)，返回 lock+cancel；拿不到 → 409 并发刷新 |
...
| `StoreToken/IsTokenValid/RevokeToken/RevokeAll` | 白名单模型：`rt:{userID}:{tokenID}` 存活即有效；RevokeAll 用 `rt:{userID}:*` 模式 SCAN 批删（改密/风控全端下线） |
