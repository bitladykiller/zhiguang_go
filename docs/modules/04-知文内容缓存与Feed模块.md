...
### 5.5 公共 Feed 的用户态一律在缓存之外叠加（已修复的跨用户串号）

**当前实现**：`assembleFromCache()` 只返回不含用户态的**公共视图**；写入 L1 的也是这份公共视图；`liked/faved` 由 `withUserState()` 在每一条读路径（L1 命中、L2 组装、锁内回源）返回前统一叠加。因此三条命中路径的响应完全一致，且共享缓存中永远不含任何用户维度字段。

**历史缺陷（务必理解，这是个真实的越权类问题）**：

公共 Feed 的 L1 键是 `feed:public:{size}:{page}:v{layout}:{version}`，**不含用户 ID**——按设计它就是全体用户共享的一份数据。而早期实现中：

1. `assembleFromCache()` 在返回前就调用了 `enrichItems()`，返回的是**已叠加当前用户 `liked/faved`** 的响应；
2. `getPublicFeedL2()` 拿到这份响应后，直接 `cacheFeedPage()` 写进了共享的 L1；
3. `getPublicFeedL1()` 命中后原样返回，不做任何用户态处理。

三步串起来的后果是：**用户 A 的点赞收藏状态被写进共享缓存，随后命中该键的用户 B 会读到 A 的状态**。这不只是「不同命中层响应不一致」，而是跨用户数据串号。

> 注意回源路径（`getPublicFeedUnderLock` 的 DB 分支）当时缓存的是 enrich **之前**的 items，所以是正确的。正因为只有 L2 这一条路径写错，问题表现得非常隐蔽：只有「先由某个登录用户触发 L2 组装、再由另一用户命中 L1」这一特定时序才会暴露。

**沉淀下来的规则**：缓存键不含用户 ID，缓存内容就不得包含任何用户维度字段。回归测试见 `TestPublicFeedL1_DoesNotLeakUserStateAcrossUsers`，它同时断言三件事：L1 落盘内容不含 `liked/faved`、第二个用户读到的是自己的状态、匿名请求不带用户态。
