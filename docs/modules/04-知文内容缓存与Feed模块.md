...
请求 → Bloom（一定不存在？）→ 404
       ...
       L1 → L2(NULL?) → 读穿锁 → DB
         ↓ 不存在
       写 NULL（兜底 Bloom 误判 / 软删残留）
```

这里有几个关键细节：

### 3.2.0 布隆过滤器 + 空值缓存（叠加，默认开启）

实现：`internal/cache/bloom.go` 的 `RedisBloom`（Redis 位图 + 双重哈希，无第三方依赖，多实例共享）。
...
| `bloom_expected_items` | 1000000 | 预估元素量 |
| `bloom_false_positive_rate` | 0.01 | 目标误判率 |
| `bloom_key` | `bloom:knowpost:ids` | Redis 位图键 |

行为：

1. **读路径最前**：`MightContain=false` → 一定不存在 → 直接 404，不打 L1/L2/DB。
2. **写路径维护**：`CreateDraft` 成功、详情回源成功 → `Add` 增量写入。
3. **启动预热**：`initKnowPost` 异步 `WarmDetailBloom`，游标扫描 `ListIDsForBloom`。
4. **冷启动 fail-open**：位图为空时一律视为可能存在，避免误拦全站。
5. **Redis 故障 fail-open**：Bloom 不可用时退回仅空值缓存路径。
6. **经典 Bloom 不能可靠删除**：软删后位仍为 1，多打一次后续路径，由 `NULL` 吸收。

为什么要和 NULL **叠加**而不是二选一：

- Bloom 擅长拦「从未出现过的扫号 ID」。
- NULL 擅长兜「查过确认不存在 / 已删除 / Bloom 误判」。
...
| `internal/cache/bloom.go` | Redis 布隆 | `RedisBloom` / `WarmDetailBloom` |
...
1. **Bloom 在最前**：专门拦扫号 ID，减少无效 L1/L2/DB。
2. **NULL 在 L2**：吸收误判、软删残留、预热未完成窗口。
3. **读穿锁**：防击穿，不是防穿透。
...
- 创建草稿就要 `Add` Bloom，否则作者马上读详情可能被拦（过滤器已预热时）。
...
| `detail_cache.bloom_key` | bloom:knowpost:ids | 位图键 |

## A7. 风险清单（主动讲）

1. L1 删 key 可能不带完整版本段（靠版本 miss 缓解）。
2. 经典 Bloom 不能可靠删除；软删依赖 NULL。
...
3. **缓存治理**：Bloom、NULL、L1/L2、版本化 key、读穿锁、热点 TTL。
...
|-----|--------------|---------------|----------|
| `POST /knowposts/draft` | `CreateDraft` | `CreateDraft` | 生成雪花 ID，写草稿，处理幂等键，加入 Bloom |
...
| `DELETE /knowposts/:id` | `Delete` | `Delete` | 软删除，事务内写 outbox |
| `GET /knowposts/:id` | `GetDetail` | `GetDetail` | Bloom + 多级缓存 + DB 回源 + 用户态增强 |
...
| 删除 | 版本失效 / NULL | feed version 变化 | 作者列表失效 | 不能删除经典 Bloom | deleted |
...
1. **Bloom 不能删除**：软删除后的 ID 仍可能被 Bloom 放行，所以必须依赖 NULL 或 DB 状态兜底。
...
> 知文模块我会按写路径和读路径分开讲。写路径里，草稿创建支持幂等键，正文上传走 OSS 直传，业务侧只保存 object key、etag、sha256 和 size；发布、删除、置顶、可见性这些会影响下游读模型的操作，都在事务内写 outbox。读路径里，详情先过 Bloom，再查 freecache 和 Redis，未命中后用 Redis 分布式锁回源 MySQL，避免击穿；不存在数据写 NULL，避免穿透。公共 Feed 用 ID 列表和 item 碎片组合，避免整页缓存大范围失效。整个模块的核心不是 CRUD，而是内容真值、缓存、搜索投影和用户态数据之间的边界。
