...
flowchart TD
    ...
    P -->|"t:{ts}:{uid} 续页"| R["rank 快路<br/>ZREVRANK 定位游标成员"]
    R -->|"成员还在"| R2["按全局排名切下一页窗口<br/>与并列规模无关"]
    R -->|"成员已被 unlike"| T
    P -->|"空(首页)"| T["score 慢路<br/>ZREVRANGEBYSCORE 包含式上界"]
...
游标前缀即路径标签：`t:` 只会续时间序（续页先 rank 快路、再退 score 慢路，见 ④/⑤）、
`u:` 只会续位图序——两条路径各自自洽，不会翻着翻着换排序。历史纯数字游标按旧语义兼容。
...
**④ 诚实的边界（离线重建已提供）：** 时间序索引只对**新互动**生效；历史数据的点赞时间本就不可考
（曾有平行键 `liker_time:*` 声称存时间，但全仓库只有读取方、从无写入方，已作为死键空间移除）。
离线重建现已落地：`RebuildLikersTimeIndex`（likers.go）全量扫位图、按批 **`ZAddNX` score=0** 灌入
时间序 ZSet——NX 保证**绝不覆盖**新互动写下的真实时间戳，可安全重复执行（幂等）；
配套 CLI 为 `cmd/rebuild-likers`（`-entity-type/-entity-id/-metric`）。重建后历史成员 `liked_at=0`、
排在所有真实时间之后，列表从「位图 uid 序」升级为「新互动时间序 + 历史 uid 序」的单一 ZSet 路径。

**⑤ 海量并列的深翻页（rank 快路）：** 重建会制造「成千上万个 score=0 成员」这种极端并列。
score 区间 + 固定并列冗余（+16）的续页方案在此失效：第 k 页要跳过 k×limit 个并列成员，
冗余只覆盖十几个，翻几页后整页都是已见成员、被误判为翻尽。因此 `t:` 游标续页先走
**`ZREVRANK` 排名定位**（`readLikersByRank`）：游标成员还在就按全局排名直接切下一页窗口，
与并列规模无关；成员被 unlike（redis.Nil）才退回 score 慢路（`readLikersByScore`，
包含式上界 + (score,member) 应用侧跳过，覆盖自然同秒规模足够）。
回归测试 `TestReadLikers_MassiveTies_DeepPagingDoesNotStall`（300 个 score=0 成员翻到第 20 页）锁定。
