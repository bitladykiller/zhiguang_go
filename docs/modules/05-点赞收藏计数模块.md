...
| `likers:{metric}:{entityType}:{entityID}` | ZSet | 点赞人**时间序索引**（score=点赞时间，toggle Lua 与位图原子同步维护） | 列表主路径 |
| `likers_cache:{entityType}:{entityID}:{metric}` | ZSet | 位图扫描结果缓存（仅索引缺失的回退路径使用） | 读优化 |
