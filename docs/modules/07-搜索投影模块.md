...
2. Canal 订阅 binlog
3. 桥接到 Kafka
4. `outbox.NewConsumer` + `SearchRowHandler` 消费 `canal-outbox`
...
sequenceDiagram
    ...
    participant C as outbox.Consumer(SearchRowHandler)
...
# 附录B：函数级源码走读（internal/search）

## B1. `projector.go` — 投影侧

| 函数 | 逐步/要点 |
|---|---|
| `ProjectPayload(ctx, raw)` | 解 payloadEnvelope（entity/op/id）→ 非 knowpost 或 id=0 跳过 → op=="delete" → `SoftDeleteKnowPost`；否则 `UpsertKnowPost` |
| `UpsertKnowPost(ctx, postID)` | `buildSearchDocument` **回查 MySQL 权威数据**（事件只是通知，投影以真值为准）→ 行不存在/已删 → 转软删 → Index 写 ES（doc id=postID） |
| `SoftDeleteKnowPost` | ES 局部更新 status="deleted"（软删保留文档，搜索层过滤） |
| `buildSearchDocument(ctx, postID)` | JOIN users 查行 → 组 SearchIndexDoc：解析 tags/imgs、补 counter 实时计数（失败零值降级）、`buildSuggestField` 组 completion 建议（标题+标签为输入源） |

## B2. `service.go` — 查询侧

| 函数 | 逐步/要点 |
|---|---|
| `NewSearchService(ctx,cfg,counter,logger)` | 建 ES 客户端 + Ping 校验 → `EnsureIndex`（不存在建索引含中文分词与 suggest mapping；存在则 `ensureCompatibleMappings` 校验兼容） |
| `Search(ctx,keyword,size,tagsCSV,after,uid)` | `parseAfter` 解 search_after 游标（base64 JSON）→ `buildSearchQuery`：multi_match(title^3/body) 包进 function_score（BM25 × like/view 热度加权），filter status=published+visible=public，`addTagFilter` terms 过滤，sort=[_score,publish_time,like_count,view_count,id] 五键稳定 → `executeSearch` → `decodeAndEnrich`（批量 BatchIsLiked/Faved 补用户态）→ `buildCursor`（取满 size 才给 next；base64(sort values)） |
| `Suggest(ctx,prefix,size)` | completion suggester（title-suggest 字段）→ 去重后返回词条 |
| `decodeAndEnrich` | hits→FeedItem 列表 + 收集 id 批查两态；uid nil 跳过 |
| `buildCursor(hits,size)` | len(hits)<size → 无下页；否则末条 sort 值序列化为游标（search_after 深分页替代 from/size） |
| `closeESBody/readESError` | 统一响应体关闭与错误文案（含 status 与 body 摘录） |

（`FeedItem` 定义在本包 `feed_item.go`：曾是 model.FeedItem 假共享层，已归还唯一使用方。）
