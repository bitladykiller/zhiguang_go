...
| `Search(ctx,keyword,size,tagsCSV,after,uid)` | `parseAfter` 解 search_after 游标（base64 JSON）→ `buildSearchQuery`：multi_match(title^3/desc/body) 包进 function_score（BM25 × like/view 热度加权），filter status=published+visible=public，`addTagFilter` terms 过滤，sort=[_score,publish_time,id] 三键稳定 → `executeSearch` → `decodeAndEnrich`（批量 BatchIsLiked/Faved 补用户态）→ `buildCursor`（取满 size 才给 next；base64(sort values)） |
