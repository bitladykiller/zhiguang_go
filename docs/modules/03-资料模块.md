...
# 附录：函数级源码走读（internal/profile）

| 函数 | 要点 |
|---|---|
| `Service.GetProfile(ctx,id)` | repo 查 model.User → `toUserProfile` DTO 映射（PasswordHash json:"-" 永不出网） |
| `Service.UpdateProfile(ctx,callerID,targetID,req)` | **callerID!=targetID → 403**（只能改自己）→ `isNoOp` 全 nil 直接成功（PATCH 语义）→ repo 动态 SET 仅更新提供字段 |
| `isNoOp(req)` | 所有指针字段皆 nil 判定 |
| `Repository.Update(ctx,id,req)` | `buildUpdateSet` 拼动态 UPDATE：`fieldDef` 切片（指针+列名）逐个非 nil 检查 → `profileUpdateFields` 白名单过滤 → 类型 switch 解引用拼 `col = ?`；无字段直接 return nil |
