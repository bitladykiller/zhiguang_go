...
sequenceDiagram
    ...
    participant API as Storage/业务 API
    participant OSS as 对象存储

    C->>API: 申请上传授权
    API-->>C: object key + PUT 预签名 URL
    C->>OSS: 直传 PUT 文件
    C->>API: ConfirmContent 元数据
    API->>API: 写主库 object key/etag/size
```
...
# 附录：函数级源码走读（internal/storage）

| 函数 | 要点 |
|---|---|
| `NewOssStorageService(cfg)` | 配置齐全才建 OSS client（endpoint/bucket/AK/SK），否则 err → handler 降级 503 |
| `GeneratePresignedPutURL(key,expiry)` | 直传签名 URL——**服务端不代理文件字节**，客户端直连 OSS；expiry 上限约束 |
| `GeneratePresignedGetURL` | 私读场景签名 GET |
| `PublicURL(key)` | 公共读拼 CDN/endpoint 前缀 |
| `GenerateObjectKey(folder,fileName)` | `{folder}/{uuid前8位}_{原文件名}`（`fmt.Sprintf("%s/%s_%s", f, uuid.New().String()[:8], fileName)`）——保留完整原文件名含扩展名，无日期分层 |
| `HeadObject(ctx,key)` | 取元数据（etag/size），供 ConfirmContent 校验客户端上报一致性 |
| `DeleteObject(ctx,key)` | 删除对象（软删场景由业务决定是否调用） |
