...
```mermaid
flowchart TD
    A[读请求] --> C[L1 freecache]
    C -->|命中| R[返回]
    C -->|miss| D[L2 Redis]
    D -->|NULL| Z[直接 miss/404]
    D -->|命中| R
    D -->|miss| B{CF.EXISTS}
    B -->|一定不存在| Z
    B -->|可能存在| E[L3 MySQL 真值]
...
因为它实现和运维复杂度更低：广播方案要处理连接、重连、消息丢失和延迟；版本号方案只需要读一个轻量 key，写路径 bump 一下版本。  
这是本模块复用知文详情的既有机制，完整论证见 [04-知文内容缓存与Feed模块](04-知文内容缓存与Feed模块.md) §9.1/Q2，这里不重复展开。
...
最大问题是 fan-out：普通用户偏写扩散，大 V 不能纯写扩散，否则一次发文产生巨量写入；最终走混合策略（小号偏写扩散，大 V 偏读扩散）。  
完整的分层论证见 [06-关注关系与Feed扩散模块](06-关注关系与Feed扩散模块.md) 场景题 3 与 [14-信息流扩散](14-信息流扩散-读扩散写扩散与推拉结合.md)，这里不重复。
...
flowchart TD
    A[读请求] --> D[多级缓存 L1/L2]
    D -->|空值| C[直接拒绝]
    D -->|未命中| B[布隆过滤器 CF.EXISTS]
    B -->|一定不存在| C
    B -->|可能存在| F[DB 回源]
    F -->|回源不存在| E[写空值]
    E --> C
    F -->|回源存在| G[写缓存并 CF.ADD]
```
...
flowchart TD
    ...
    B --> B1[两级缓存 miss 后 CF.EXISTS 拦一定不存在]
    B --> B2[NULL 兜误判和软删]
    C --> C1[分布式读穿锁]
    C --> C2[double check]
    D --> D1[TTL jitter]
