...
```mermaid
flowchart TD
    A[请求命中热点 Key] --> B[本地 HotKeyDetector 缓冲计数]
    B --> C[定期 1s Flush 到 Redis 聚合]
    C --> D[滑动窗口热度计算 QPS]
    D --> E{热度等级划分}
    E -->|QPS 较低| E1[TTLLow: 30s]
    E -->|QPS 中等| E2[TTLMedium: 60s]
    E -->|QPS 极高| E3[TTLHigh: 300s]
    E1 & E2 & E3 --> F[Lua 脚本执行原子更新]
    F --> G{TargetTTL > CurrentTTL?}
    G -->|是| H[延长 TTL]
    G -->|否| I[保持现状, 绝不缩短]
```
...
1. **本地轻量缓冲**：API 节点在处理请求时，优先在进程内 `HotKeyDetector` 进行计数累加，避免每次请求都发起网络 API 调 Redis `EXPIRE` 命令。
2. **近实时 Redis 聚合**：后台 Runner 定期（如每 1s）将本地滑块计数 Flush 到 Redis Hash，实现多 API 实例访问量的近实时全局聚合。
3. **分级 TTL 计算**：根据当前 QPS 将热度划分为 Low / Medium / High 三个等级，分别对应不同的目标 TTL。
4. **Lua 脚本防竞态（只增不减原则）**：在调用 Redis 延长 TTL 时，必须通过 Lua 脚本原子的对比 `CurrentTTL` 与 `TargetTTL`。只有当 `TargetTTL > CurrentTTL` 时才更新，**绝不缩短已有 TTL**。这防止了不同 API 实例因采样偏差导致“高热度 TTL 被误缩短”的隐患。
...
1. **切忌直接在每次 API 请求时调用 Redis `EXPIRE` 尝试延长热点 Key**：
   - 错误做法：每次 `GetDetail` 命中 Redis 缓存时，直接执行 `rdb.Expire(ctx, key, ttl)`。
   - 后果：若单接口 QPS 达 5 万，每次访问触发额外的网络 RTT 去调用 `EXPIRE`，直接将 Redis CPU 线程打满！
   - 正确做法：必须在 API 进程内通过 `HotKeyDetector` 缓冲滑动窗口，按固定时间间隔（如 1 秒）批次提交。

2. **多实例并发延长 TTL 时必须使用 Lua 确保“只增不减”**：
   - 错误做法：实例 A 判定热度为 High (设置 TTL=300s)，实例 B 在零点几毫秒后因为采样较少判定为 Low (执行 EXPIRE 30s)。
   - 后果：实例 B 覆盖了实例 A 的设置，原本应该存活 300s 的爆款 Key 在 30s 后陡然失效导致爆破回源！
...
- **思考题 2**：如果某个 API 节点被强行 kill -9 导致 `HotKeyDetector` 本地 map 中的 1 秒计数丢失，影响大吗？
  - *提示*：影响极小。多节点分布式部署下，Redis Hash 承接了全站滑动窗口的近实时聚合，单个节点的少量本地计数丢失仅导致毫秒级热度微小微调，系统会自动容错。
