...
2. **当前窗口实现有一个必须知晓的边界**：`flushOnce` 将桶计数 `HINCRBY` 到 `hotwin:{cacheKey}` 后，会删除 `nowBucket-1` 到 `nowBucket-10` 的字段。因此当前代码通常只保留当前桶，而没有实际保留 10 个历史桶；阈值 `50/200/500` 在运行时更接近“当前约 6 秒桶内的命中数”，不能在文档或告警中按严格 60 秒窗口解读。若要实现设计中的 60 秒窗口，清理逻辑应只删除窗口之外的更旧桶。
...
4. **当前实现仍有命中路径 Redis IO**：知文详情调用双 key Lua、Feed item 调用单 key Lua 来做只增不减延长；它们在每次相应缓存命中时都会执行。局部计数不写 Redis，但 TTL 延长并未被批处理，这是当前实现与理想方案的区别。
...
`levels` 是进程内 map。一次 flush 只会重算本轮 `snapshot` 中出现的 key；没有新命中的 key 不会被主动从这个 map 清掉。与此同时，Redis 的 `hotkey:active:*` 标记默认 60 秒后过期，但它只在本地没有等级时作为 Medium 回退。

因此热度降温不是严格实时的：持续访问的 key 会在下一次 flush 被重新评估；停止访问的 key 可能在本地保留旧等级，直到该 key 再次进入 snapshot 后才被覆盖。TTL 延长是性能增强而非业务正确性依赖，这个近似边界可以接受，但应监控异常长的缓存寿命。
...
sequenceDiagram
    ...
    Flush->>Flush: sumBucketsInWindow（通常只剩当前桶）
...
2. 随后的 `TtlForPublic()` 在本地没有等级时会查一次 `hotkey:active:*`；详情缓存命中还会执行双 key Lua，Feed item 命中会执行单 key Lua。因此不能把完整命中路径描述为“零 Redis IO”。
3. 跨实例计数通过 Redis Hash 完成；`hotkey:active:*` 是短期兜底标记，其他实例命中它时一律按 Medium 回退，而不是恢复原始 Low/High 等级。
4. HotKeyDetector 只计算热度和目标 TTL，真正延长缓存 TTL 的 Lua 在 knowpost 缓存层执行。
5. 配置虽声明 10 个桶，但当前清理循环会移除前 10 个桶字段；实际统计窗口通常约为当前 6 秒桶，而不是 60 秒。这里是实现现状，不能用“滑动 60 秒窗口已落地”来表述。
...
- **思考题 2**：如果某个 API 节点被强行 kill -9，导致 `HotKeyDetector` 本地 map 中尚未 flush 的计数丢失，影响大吗？
  - *提示*：会丢失该节点最多一个 flush 间隔内的计数，默认约 6 秒；已写入 Redis 的数据和其他节点的计数仍在。结果可能是热点升级延后或短暂降级，而不是业务数据错误。还应注意当前桶清理实现通常只保留当前桶，因此这类丢失对临界阈值的影响会比设计中的 60 秒窗口更敏感。
