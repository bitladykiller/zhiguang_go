...
0. **RedisBloom Cuckoo CF.*（可选/默认开，配置键 bloom_*）**：共享存在性前置，支持 `CF.DEL`
1. L1：进程内 freecache
...
知文详情默认 **RedisBloom CF.* + NULL 叠加**（见 `docs/modules/04-知文内容缓存与Feed模块.md` 与 `internal/cache/bloom.go`）。
...
4. 实现依赖 `redis-stack-server` 的 RedisBloom 模块（`internal/cache/bloom.go`）。
