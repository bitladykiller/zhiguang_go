...
0. **Cuckoo Filter（可选/默认开，配置键 bloom_*）**：Redis 共享存在性前置判断，支持 Delete
1. L1：进程内 freecache
...
- Cuckoo 拦「一定不存在」的扫号，减穿透；软删可从过滤器移除
- NULL 兜「查过不存在 / 过滤器误判 / 删除失败边界」
- L1 解决单实例极低延迟
- L2 解决跨实例共享
- L3 保证最终真值

知文详情默认 **Cuckoo + NULL 叠加**（见 `docs/modules/04-知文内容缓存与Feed模块.md` 与 `internal/cache/bloom.go`）。
...
> 缓存按访问模式选型。穿透用 Cuckoo 加空值，击穿用读穿锁，热点用滑动窗口延长 TTL，失效用版本号解决多实例 L1 残留。
...
flowchart TD
    ...
    E -->|不存在或软删| F[写 NULL]
    F --> C
    E -->|存在| G[写缓存并补 Cuckoo Add]
```

Cuckoo 和 NULL 解决的是不同问题：

1. Cuckoo：适合拦“从未存在过”的随机扫号，且软删可 `Delete`。
2. NULL：适合兜误判、删除失败、DB 确认不存在、预热窗口。
3. 过滤器故障时 fail-open，不误拦真实内容。
4. 实现为 Redis 字节表 + Lua（`internal/cache/bloom.go`），无 Redis 模块依赖。
...
| Cuckoo 故障 | 不能前置拦截 | fail-open，继续多级缓存 |
