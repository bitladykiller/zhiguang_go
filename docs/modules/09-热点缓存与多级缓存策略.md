...
0. **Bloom（可选/默认开）**：Redis 共享存在性前置判断
1. L1：进程内 freecache
...
- Bloom 拦「一定不存在」的扫号，减穿透
- NULL 兜「查过不存在 / Bloom 误判 / 软删残留」
- L1 解决单实例极低延迟
- L2 解决跨实例共享
- L3 保证最终真值

知文详情默认 **Bloom + NULL 叠加**（见 `docs/modules/knowpost.md` 与 `internal/cache/bloom.go`）。
...
> 缓存按访问模式选型。穿透用 Bloom 加空值，击穿用读穿锁，热点用滑动窗口延长 TTL，失效用版本号解决多实例 L1 残留。
...
Bloom 和 NULL 解决的是不同问题：

1. Bloom：适合拦“从未存在过”的随机扫号。
2. NULL：适合兜 Bloom 误判、软删除、DB 确认不存在。
3. Bloom 故障时 fail-open，不误拦真实内容。
4. 经典 Bloom 不能可靠删除，所以删除后的内容仍要靠 NULL 和 DB 状态兜底。
...
| Bloom 故障 | 不能前置拦截 | fail-open，继续多级缓存 |
