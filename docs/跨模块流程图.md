...
| 写扩散 fanout | 发布推粉丝 timeline | **半接线**：有 `FanoutPublisher`/`FanoutConsumer`/`Service`，但 **Publisher 未被 knowpost/bootstrap 调用**；Canal 只写 `canal-outbox`，**不写** `fanout` topic | 讲「算法与读侧兜底已有，生产端未闭环」 |
...
flowchart TB
    ...
    subgraph 半接线_缺口
      C1[FanoutPublisher 未注入写路径]
      C2[fanout topic 无生产端]
      C3[FanoutConsumer 可启动但无消息]
      C1 -.-> C2 -.-> C3
    end
...
5. **fanout topic 与 canal-outbox 是两条 topic**；当前发布路径不会自动往 `fanout` 灌消息。  
...
| fanout → relation | 写扩散取粉丝列表 | `FanoutService` | 消费 `fanout` topic（**生产端缺口**） |
...
> **注意**：本节只画**已闭环**的搜索投影链路。写扩散见 §6（含接线缺口）。
...
5. rebuild / epoch fence 防 like/fav 旧 delta 污染新快照。  
...
```mermaid
flowchart TD
    A[发布成功] --> B[FanoutPublisher.Publish]
    B --> C[Kafka topic: fanout]
    C --> D[FanoutConsumer]
    D --> E[relation 拉粉丝]
    E --> F[pipeline ZADD timeline:粉丝ID]
```
...
```mermaid
flowchart TD
    A[knowpost 发布] --> B[同事务 outbox]
    B --> C[Canal → canal-outbox]
    C --> D[search / relation consumers]
    A -.->|未调用| E[FanoutPublisher]
    F[initFanout 仅注册 Consumer] --> G[订阅 topic fanout]
    E -.->|无生产消息| G
    G --> H[timeline 通常为空]
    H --> I[读侧走 public/mine 缓存与 DB]
```

| 组件 | 状态 |
|------|------|
| `fanout.Service.FanoutPost` | 有实现：批量 timeline、大 V 截断、TTL |
| `fanout.FanoutPublisher` | 有实现；**全仓库无业务调用点** |
| `fanout.FanoutConsumer` | brokers 非空时 **会启动** |
| Canal Bridge | **只写** `canal-outbox`，**不写** `fanout` |
| `init_fanout.go` 注释 | 写 outbox→Canal→fanout；**与代码不符** |

### 6.2 读 Feed（以当前 API 为准）

```mermaid
flowchart TD
    A[Feed 请求] --> B{接口}
    B -->|GET feed/public| C[公共 Feed<br/>L1/L2 碎片缓存 → DB]
    B -->|GET feed/mine| D[GetMineFeed]
    D --> E{timeline:userID 有数据?}
    E -->|有| F[按 post_id 批量查详情]
    E -->|无/失败| G[降级 GetMyPublished<br/>本人已发布列表]
...
1. **公共 Feed / 我的已发布** 是主读路径；timeline 是写扩散收件箱投影。  
2. `GetMineFeed` 空 timeline 时回落 **本人已发布**——写扩散未闭环时，**不是**完整「关注流读扩散」。  
3. 面试应说清：**算法与 consumer 在，生产端未把发布事件灌进 fanout topic**。  
...
| `FanoutConsumer` | Kafka brokers 非空 | 消费 `fanout`（**生产端未闭环**） |
...
flowchart TD
    ...
    subgraph fanout_半接线
      M[Publisher 未注入] -.-> N[topic fanout]
      N -.-> O[Consumer 可空转]
    end
```
...
### 场景 A：用户注册后发第一篇知文（搜索已闭环；粉丝 timeline 未闭环）
...
sequenceDiagram
    ...
    ASYNC->>SCH: 索引可搜（最终一致）
    FE->>SCH: 搜索可见新帖
    Note over KP: 当前未调用 FanoutPublisher<br/>粉丝 timeline 不会自动被推送
```
