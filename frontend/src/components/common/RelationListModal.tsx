...
const RelationListModal = ({ open, onClose, userId, mode }: RelationListModalProps) => {
    ...
    const run = async () => {
      ...
      try {
        const resp = mode === "following"
          ? await relationService.following(userId, initialLimit, 0, tokens.accessToken)
          : await relationService.followers(userId, initialLimit, 0, tokens.accessToken);
        if (cancelled) return;
  ...
  const loadMore = async () => {
    ...
    try {
      const resp = mode === "following"
        ? await relationService.following(userId, initialLimit, offset, tokens.accessToken)
        : await relationService.followers(userId, initialLimit, offset, tokens.accessToken);
      const list = Array.isArray(resp) ? resp : [];
